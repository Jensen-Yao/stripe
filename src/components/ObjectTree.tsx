...
export function ObjectTree() {
  ...
  const items: TreeItem[] = [
    ...
    ...groundAssets.map((item): TreeItem => ({ key: item.id, depth: 1, label: item.name, detail: item.kind === "station" ? "地面站" : "目标点", selection: { kind: "groundAsset", id: item.id }, icon: <MapPin size={14} /> })),
  ...
  return (
    <aside className="object-panel">
      <div className="panel-title"><strong>对象浏览器</strong><div className="panel-actions"><button title="新增卫星" onClick={() => useWorkbenchStore.getState().addSpacecraft()}><Orbit size={13} /><Plus size={10} /></button><button title="新增目标点" onClick={() => useWorkbenchStore.getState().addGroundAsset({ id: makeId("ground"), name: `目标 ${groundAssets.length + 1}`, kind: "target", visible: true, location: { lon: 116, lat: 40, heightKm: 0 }, minElevationDeg: 10 })}><MapPin size={13} /><Plus size={10} /></button><span>{spacecraft.length + stripes.length + groundAssets.length + tasks.length}</span></div></div>
